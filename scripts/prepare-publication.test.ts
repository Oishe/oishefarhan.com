...
test("rejects Quarto callouts, which need a Bootstrap this site does not load", async () => {
  const vault = await withVault({
    "notes/computed.qmd": note(
      "Computed",
      "publish: true\n",
...
// Fixtures exercise the Quarto bridge -- a Python cell, a Jupyter widget, a
// rendered fragment inside a Quartz shell -- and nothing in the portfolio does.
// So they must still be built, and must never reach the public site. The whole
// separation rests on one flag, which is why it is tested from both sides.
test("fixtures stay out of the production build", async () => {
  ...
  assert.deepEqual(await readdir(vault.contentDir), ["index.md"])
})

test("the validation build stages fixtures as published", async () => {
  const vault = await withVault({
    "index.md": note("Home", "publish: true\n", "Real content."),
    "examples/fixture.md": note("Fixture", "publish: false\nfixture: true\n", "Bridge check."),
  })

  const result = await vault.run({ includeFixtures: true })

  assert.deepEqual(
    result.published.map((document) => document.slug).sort(),
    ["examples/fixture", "index"],
  )
  // Quartz reads only the staged tree, and explicit-publish drops anything that
  // does not say so. The vault source keeps `publish: false`; the staged copy
  // must not, or the fixture build renders nothing.
  const staged = await readFile(path.join(vault.contentDir, "examples/fixture.md"), "utf8")
  assert.match(staged, /^publish: true$/m)
  assert.doesNotMatch(staged, /^publish: false$/m)
})
